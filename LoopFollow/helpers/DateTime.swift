...
class dateTimeUtils {
    ...
    //Auggie added this for use with CGMs (Anubis, Libre, future G7, etc)that can go more than 10 days back - extend it to 20 days
    static func nowMinus20DaysTimeInterval() -> String {
        let today = Date()
        let oldDate = Calendar.current.date(byAdding: .day, value: -20, to: today)!
        let dateFormatter = DateFormatter()
        dateFormatter.dateFormat = "yyyy-MM-dd"
        dateFormatter.locale = Locale(identifier: "en_US")
        dateFormatter.timeZone = TimeZone.init(secondsFromGMT: 0)
